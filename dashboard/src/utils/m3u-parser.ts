...
export function parseM3U(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("#EXTINF:")) {
      ...
      if (baseUrl && !finalUrl.startsWith('http://') && !finalUrl.startsWith('https://')) {
        try {
...
export async function fetchAndParseM3U(
  ...
  const playlist = parseM3U(content, playlistName, url);
  playlist.url = url;
...
export function parsePlaylist(
  ...
  if (trimmed.startsWith("#EXTM3U") || trimmed.includes("#EXTINF:")) {
    return parseM3U(content, playlistName, baseUrl);
  }
...
export async function fetchAndParsePlaylist(
  ...
  try {
    const playlist = parsePlaylist(content, playlistName, url);
    playlist.url = url;
    return playlist;
