# World Cup IPTV Channels

Here are the direct links to all the available IPTV playlists. You can copy any of these URLs and paste them directly into your IPTV player!

### 🌟 Master Playlist (All Channels Combined)
Loads all **2,981+ channels** from every playlist below in one go:
```text
https://raw.githubusercontent.com/Joy43/OpenTv/main/worldcup/index.m3u
```

---

### 📂 Individual Playlists

**1. TV Networks Playlist** (1091 Channels)
```text
https://raw.githubusercontent.com/Joy43/OpenTv/main/opentv-app/worldcup/tvnetwork.m3u
```

**2. World Cup Playlist**
```text
https://raw.githubusercontent.com/Joy43/OpenTv/main/opentv-app/worldcup/WorldCup.m3u
```

**3. Only FIFA Playlist**
```text
https://raw.githubusercontent.com/Joy43/OpenTv/main/opentv-app/worldcup/OnlyFifa.m3u
```
